...
SkeletonViewerClipped::SkeletonViewerClipped()
   : mBonesVAO(0)
   , mBonesVBO(0)
   , mJointsVAO(0)
   , mJointsVBO(0)
   ...
   , mBoneColors()
{
   ...
   mBoneShader = ResourceManager<Shader>().loadUnmanagedResource<ShaderLoader>("resources/shaders/bone_clipped.vert",
                                                                               "resources/shaders/bone_clipped.frag");

   mJointShader = ResourceManager<Shader>().loadUnmanagedResource<ShaderLoader>("resources/shaders/joint_clipped.vert",
                                                                                "resources/shaders/joint_clipped.frag");
...
SkeletonViewerClipped::SkeletonViewerClipped(SkeletonViewerClipped&& rhs) noexcept
...
SkeletonViewerClipped& SkeletonViewerClipped::operator=(SkeletonViewerClipped&& rhs) noexcept
{
   mBonesVAO         = std::exchange(rhs.mBonesVAO, 0);
   mBonesVBO         = std::exchange(rhs.mBonesVBO, 0);
   mJointsVAO        = std::exchange(rhs.mJointsVAO, 0);
...
void SkeletonViewerClipped::InitializeBones(const Pose& pose)
{
   ResizeBoneContainers(pose);
   ConfigureBonesVAO(mBoneShader->getAttributeLocation("inPos"), mBoneShader->getAttributeLocation("inCol"));
}

void SkeletonViewerClipped::UpdateBones(const Pose& animatedPose, const std::vector<glm::mat4>& animatedPosePalette)
{
   int posIndex = 0;
   int colorIndex = 0;
...
void SkeletonViewerClipped::LoadBoneBuffers()
{
...
// TODO: Experiment with GL_STATIC_DRAW, GL_STREAM_DRAW and GL_DYNAMIC_DRAW to see which is faster
void SkeletonViewerClipped::LoadJointBuffers()
{
...
void SkeletonViewerClipped::ConfigureBonesVAO(int posAttribLocation, int colorAttribLocation)
{
   glBindVertexArray(mBonesVAO);
...
void SkeletonViewerClipped::ConfigureJointsVAO(int posAttribLocation, int normalAttribLocation)
{
   glBindVertexArray(mJointsVAO);
...
void SkeletonViewerClipped::RenderBones(const Transform& model, const glm::mat4& projectionView, const glm::vec2& horizontalClippingPlaneYNormalAndHeight)
{
   ...
   mBoneShader->setUniformVec2("horizontalClippingPlaneYNormalAndHeight", horizontalClippingPlaneYNormalAndHeight);
   glBindVertexArray(mBonesVAO);
...
void SkeletonViewerClipped::RenderJoints(const Transform& model, const glm::mat4& projectionView, const std::vector<glm::mat4>& animatedPosePalette, const glm::vec2& horizontalClippingPlaneYNormalAndHeight)
{
   ...
   mJointShader->setUniformVec2("horizontalClippingPlaneYNormalAndHeight", horizontalClippingPlaneYNormalAndHeight);
   glBindVertexArray(mJointsVAO);
...
void SkeletonViewerClipped::ResizeBoneContainers(const Pose& animatedPose)
{
